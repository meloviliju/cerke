...
function gain_ap(target_id) {
    ...
    if (piece.classList.contains("right")) {
        sendToRight(target_id);
    }
    else if (piece.classList.contains("down")) {
        sendToDown(target_id);
    }
    else if (piece.classList.contains("left")) {
        sendToLeft(target_id);
    }
    else {
        sendToUp(target_id);
    }
...
function sendTo_ap(dest, piece_id) {
...
function sendToRest_ap(piece_id) {
    const piece = getNth(piece_id);
    piece.classList.remove("up");
    piece.classList.remove("left");
    piece.classList.remove("down");
...
function spawnTo_ap(dest, piece_img_name) {
    const destination = document.getElementById(dest);
...
function spawnToUp(piece_img_name) { spawnTo_ap("up", piece_img_name); }
function spawnToRight(piece_img_name) { spawnTo_ap("right", piece_img_name); }
function spawnToDown(piece_img_name) { spawnTo_ap("down", piece_img_name); }
function spawnToLeft(piece_img_name) { spawnTo_ap("left", piece_img_name); }
...
const tdClickCallbackFunction_ap = (newtd, newid) => () => {
...
const pieceClickCallbackFunction_ap = (piece_num) => () => {
    if (choice.value !== null)
        gain_ap(piece_num);
    else
...
function setupMaterials_ap() {
    loadBoard(tdClickCallbackFunction_ap);
    loadRestArea();
    loadPieces(pieceClickCallbackFunction_ap);
    loadPieceList();
}
    ...
    document.addEventListener("keydown", (event) => {
        switch (event.key) {
            ...
            case "a":
                ...
                break;
            case "y":
                if (window.confirm('四人机戦の初期配置に並べます、よろしいですか？')) {
                    init_ap();
                }
                break;
        }
